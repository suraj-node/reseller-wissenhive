                        ...
                        @php $count = 1 @endphp
                            @foreach($invoices as $inv)
                                <tr>
                                    ...
                                    <td class="cell">{{ $inv->candidate_name }}</td>
                                    <td class="cell">{{ $inv->reseller_name }}</td>
                                    <td class="cell">{{ $inv->amount }}</td>
                                    <td class="cell">{{ $inv->course }}</td>
                                    <td class="cell">
                                        
                                        @if($inv->status == NULL)
                                            <p class="badge bg-warning">Pending</p>
                                        @elseif($inv->status == 1)
                                            <p class="badge bg-success badge-sm">Approve</p>
                                        @else
                                            <p class="badge bg-danger badge-sm">Disapprove</p>
                                        @endif
                                    </td>
                                    <td class="cell">{{ $inv->reseller_comment }}</td>
                                    <td class="cell">
                                        <a href="javascript:void(0)" onclick="generateForm('{{ $inv->id }}', 2)" class="btn btn-danger btn-sm">Disapprove</a>
                                        <a href="javascript:void(0)" onclick="generateForm('{{ $inv->id }}', 1)" class="btn btn-success btn-sm">Approve</a>
                                        <a href="{{ route('reseller.admin-invoice-remove', ['invoice_id'=>$inv->id]) }}" class="btn btn-info btn-sm">Remove</a>
                                    </td>
